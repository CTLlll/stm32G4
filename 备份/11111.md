...
#define SINE_RESOLUTION 511 
#define PWM_FREQUENCY 50     // ?? PWM ????,?? Hz
#define TIMER_CLOCK 170000000
uint32_t period_set_A;   
uint16_t sine_wave[SINE_RESOLUTION];
uint32_t phase_shift_values[SINE_RESOLUTION];

volatile uint32_t sine_index = 0;  
#define SINE_PERIOD_MS (1000 / PWM_FREQUENCY)  // ?????,?? ms
uint32_t sine_time_interval_us;
void Calculate_Sine_Time_Interval() {
    // ??????,????????(??)
    sine_time_interval_us = (SINE_PERIOD_MS * 1000) / SINE_RESOLUTION;
}
...
​	
	...
	void phase_shift_PWM(uint32_t sine_index) {
	HRTIM_CompareCfgTypeDef pCompareCfg = {0};
	
	// ????????????
	uint32_t phase_shift = phase_shift_values[sine_index];
	...
	// ????????(TB2),???????? 180°
	pCompareCfg.CompareValue = (phase_shift + (period_set_A / 2)) % period_set_A; // ?????????
	...
	void Generate_Sine_Wave_and_Phase_Shift() {
    // ?? PWM ?????(????? / PWM ????)
    period_set_A = TIMER_CLOCK / PWM_FREQUENCY;

    for (int i = 0; i < SINE_RESOLUTION; i++) {
        // ???? [0, 360°),???????
        float angle = (2.0f * M_PI * i) / SINE_RESOLUTION;
    
        // ??????,?? 0 ~ 4095,?? DAC ????
        sine_wave[i] = (uint16_t)((sin(angle) + 1.0f) * 2047);
    
        // ???????,?? 0 ~ period_set_A / 2
        float sin_value = sin(angle); // ??? -1.0 ~ 1.0
        phase_shift_values[i] = (uint32_t)((sin_value + 1.0f) * (period_set_A / 4)); // ??? 0 ~ ???
    }
}
	void setup_original_PWM() {
    HRTIM_CompareCfgTypeDef pCompareCfg = {0};

    // ???????(TA1)? 50% ???
    pCompareCfg.CompareValue = period_set_A / 2;
    HAL_HRTIM_WaveformCompareConfig(&hhrtim1, HRTIM_TIMERINDEX_TIMER_A, HRTIM_COMPAREUNIT_1, &pCompareCfg);
...
//    // ??????????? TB2(???? 180°)
//    pCompareCfg.CompareValue = phase_shift + (period_set_A / 2);
//    HAL_HRTIM_WaveformCompareConfig(&hhrtim1, HRTIM_TIMERINDEX_TIMER_B, HRTIM_COMPAREUNIT_2, &pCompareCfg);
//    HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TB2);
//}
// ???:??????????????
...
//    while (1) {
//        // ????????? PWM ????
//        phase_shift_PWM(sine_index);

//        // ????????????
//        sine_index = (sine_index + 1) % SINE_RESOLUTION;

//        // ?????????(20ms / SINE_RESOLUTION)
//        HAL_Delay(20 / SINE_RESOLUTION); // ?????????,??????????
//    }
//}
...
/**
  ...
  * @retval int
  ...
  /* USER CODE BEGIN 2 */
   //Generate_Sine_Wave();
    Generate_Sine_Wave_and_Phase_Shift();
	
	Calculate_Sine_Time_Interval();
