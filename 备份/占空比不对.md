...
void Update_PWM_Phase(uint16_t sine_value, uint32_t input_capture_value);
...
void Generate_Sine_Wave(void) {
    for (int i = 0; i < SINE_RESOLUTION; i++) {
        // ?????,???? [0, 4095]
        sine_wave[i] = (uint16_t)((sin(2 * M_PI * i / SINE_RESOLUTION) + 1) * 2047);  // ??? 12 ???
    }
}
...
void start_module_A(void) {
    ...
    HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TC1 | HRTIM_OUTPUT_TC2);  // ?? PWM ???
}
...
void stop_module_A(void) {
    ...
    HAL_HRTIM_WaveformCounterStop(&hhrtim1, HRTIM_TIMERID_MASTER | HRTIM_TIMERID_TIMER_A | 
                                      HRTIM_TIMERID_TIMER_B | HRTIM_TIMERID_TIMER_C);
}

void Update_PWM_Phase(uint16_t sine_value, uint32_t unused_input_capture_value) {
