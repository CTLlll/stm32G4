...
//	  phase_offset += 100; // ?????????
//        if (phase_offset >= htim1.Init.Period) {
//            phase_offset = 0; // ???????????
//        }

        // ?? CCR2 ????????
        uint32_t new_ccr2 = (850 + phase_offset) % htim1.Init.Period;

        // ?? CH2 ????(CCR2)
        __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, new_ccr2);
